...
machillo([X|T],R):-X= pequeno,machillo(T,R).
...
karina([],"Karina Ramos").
karina([X|T],R):-X= actriz,karina(T,R).
karina([X|T],R):-X= mujer,karina(T,R).
karina([X|T],R):-X= no,karina(T,R).
karina([X|T],R):-X= alta,karina(T,R).
karina([X|T],R):-X= 'heredia',karina(T,R).
karina([X|T],R):-X= siguiente ,karina(T,R).

melissa([],"Melissa Mora").
melissa([X|T],R):-X= cantante,melissa(T,R).
melissa([X|T],R):-X= actriz,melissa(T,R).
melissa([X|T],R):-X= mujer,melissa(T,R).
melissa([X|T],R):-X= no,melissa(T,R).
melissa([X|T],R):-X= alta,melissa(T,R).
melissa([X|T],R):-X= 'alajuela',melissa(T,R).
melissa([X|T],R):-X= siguiente ,melissa(T,R).

victor([],"Victor Carvajal").
victor([X|T],R):-X= presentador,victor(T,R).
victor([X|T],R):-X= hombre,victor(T,R).
victor([X|T],R):-X= normal,victor(T,R).
victor([X|T],R):-X= no,victor(T,R).
victor([X|T],R):-X= alajuela,victor(T,R).
victor([X|T],R):-X= siguiente ,victor(T,R).
...
revisar(L,R,N):- karina(L,R),append([],R,L_r), Nn is N-1,revisar(L,L_r,Nn).
revisar(L,R,N):- melissa(L,R),append([],R,L_r), Nn is N-1,revisar(L,L_r,Nn).
revisar(L,R,N):- victor(L,R),append([],R,L_r), Nn is N-1,revisar(L,L_r,Nn).
...
iniciar:-
    ...
    write("Recuerde escribir en minuscula todo y con comillas ('')"),nl,
